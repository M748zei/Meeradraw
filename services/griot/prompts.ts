...
1. Première ligne : une affirmation forte + un drapeau du pays concerné (seul emoji autorisé, uniquement dans les accroches, jamais dans le script parlé).
...
- Si tu n'es pas certain d'un fait : formulation prudente dans le script (« selon plusieurs sources », « les estimations vont de X à Y ») ET une entrée précise dans a_verifier.
...
SORTIE — un objet JSON STRICT, sans texte autour, sans balises markdown, avec EXACTEMENT ces clés :
{
  ...
  "description": "description Facebook prête à coller, SANS hashtags",
...
export function buildRecitUserPrompt(input: RecitInput, contexteWeb?: string | null): string {
  ...
  const lignes = [
    ...
    `DURÉE CIBLE : ${cible.secondes} secondes — environ ${cible.mots} mots parlés, ${cible.plans} plans.`,
  ];
  if (contexteWeb) {
    lignes.push(
      "",
